...
@app.route("/send-whatsapp", methods=["POST"])
def send_whatsapp():
    # Accept form data (POSTed from frontend)
    form_data = request.form.to_dict()
    ...
    recipient_number = user.get("whatsapp_number")
    recipient_number = re.sub(r"\D", "", recipient_number or "")  # Clean non-digit characters
    logging.info(f"Cleaned recipient number: {recipient_number}")
